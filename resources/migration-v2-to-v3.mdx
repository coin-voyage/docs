...
description: "Upgrade a CoinVoyage v2 integration to the v3 beta API and PayKit 3.0.0-beta.5 SDK surface."
layout:
  description:
    visible: false
---
...
<Info>
  v3 is currently tagged **Beta** in the docs version selector. Keep your v2 integration running until you have tested order creation, payment completion, webhooks, refunds, swaps, and any off-ramp workflows in your target environment.
</Info>
...
| OpenAPI source | `/v2/openapi.json` | `/v3/openapi.json` |
...
| Standalone swap execution | `swapData()` | `swapExecute()` |
...
| Fiat payout API wording | KYC and withdrawals | Off-ramp verification and off-ramp intents |

## Migration checklist

1. Upgrade PayKit to `@coin-voyage/paykit@3.0.0-beta.5`.
2. Change direct API calls from `/v2` to `/v3`.
3. Replace `ApiClient` PayOrder helper names with the v3 Order helper names.
4. Update `PayButton` and `PayButton.Custom` from `payId` to `orderId`.
5. Replace `onPaymentStarted` with the more specific payment lifecycle callbacks.
6. Update webhook handlers to read `{ event, delivered_at, order }`.
7. Rename KYC and withdrawal API calls to the off-ramp verification and off-ramp intent methods.
8. Update standalone swap execution from `swapData()` to `swapExecute()`.
9. Run a full test payment and verify that webhook fulfillment remains idempotent.

## Upgrade PayKit

```bash
pnpm add @coin-voyage/paykit@3.0.0-beta.5
```
