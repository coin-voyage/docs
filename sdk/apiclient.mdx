...
Retrieves token balances and USD values for 1–10 wallets. Each successful scan
appears in `data.portfolio`; a wallet-specific failure appears in `data.errors`
without discarding successful results for the other wallets.
...
`wallets` is required and accepts between 1 and 10 entries. Set
`force_refresh: true` when you need a fresh scan instead of a cached result.

<Warning>
  **Migrating from 3.2:** replace `wallet: { ... }` with
  `wallets: [{ ... }]`. The response is now grouped into `portfolio` and
  `errors` arrays, even when you request one wallet.
</Warning>
...
Use these API-key methods for Stripe embedded on-ramp flows. Card Payments
support `"usd"` and `"eur"` as `source_currency` values for eligible
...
For a standalone EUR-funded Card Payment, set `source_currency` to `"eur"`:
