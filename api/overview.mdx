...
<Warning>
  Generate authorization signatures on your server only. A leaked API secret can create sales, refunds, invoices, off-ramp intents, webhooks, and fee claims for your organization.
</Warning>
...
See [Invoices API](/invoices/api) for signed invoice creation, draft, list, retrieval, archive, and delete operations.
...
5. Update internal state from outcome and recovery events such as `ORDER_COMPLETED`, `ORDER_REFUNDED`, `ORDER_EXPIRED`, `ORDER_ERROR`, or `ORDER_PARTIAL_PAYMENT`.
